...
void sync_enqueue(ThreadPool* pool, Task* task) {
    printf("start %s\n", __func__);
    pthread_mutex_t* mutex = &pool->queueMutex;
...
int sync_getAcceptsTasks(ThreadPool* pool) {
    ...
    int res;
    printf("1\n");
    pthread_mutex_t* mutex = &pool->acceptingMutex;
    pthread_mutex_lock(mutex);
    printf("2\n");
    res = pool->acceptsTasks; // PROBLEMMMMMM
    printf("3\n");
    pthread_mutex_unlock(mutex);
    printf("end %s\n", __func__);
    return res;
...
void* threadRoutine(void* threadPool) {
    ...
    while (sync_getAcceptsTasks(threadPool)) {
        if (!sync_isEmpty(threadPool)) {
            ...
            pthread_mutex_t* mutex = &pool->taskAddedMutex;
            pthread_mutex_lock(mutex);
            pthread_cond_wait(&pool->taskAddedCond, mutex);
            pthread_mutex_unlock(mutex);
        } else {
...
int tpInsertTask(ThreadPool* threadPool, void (*computeFunc) (void *), void* param) {
    ...
    pthread_mutex_t* mutex = &threadPool->taskAddedMutex;
    pthread_mutex_lock(mutex);
